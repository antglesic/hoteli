...
$head = "<thead>" . "<tr>" . "<th>Ime i prezime</th>" . "<th>Email</th>" . "<th>Korisničko ime</th>" . "<th>Status</th>" . "<th>Otključaj</th>" . "<th>Zaključaj</th>" . "</tr>" . "</thead>";
$table = "";

while ($row = $korisnici->fetch_assoc()) {
    $table = $table . "<tr>";
    if ($row["status"] == '0') {
        $table = $table . "<td>" . $row["ime"] . " " . $row["prezime"] . "</td>" . "<td>" . $row["email"] . "</td>" . "<td>" . $row["korisnicko_ime"] . "</td>" . "<td>Zaključan</td>" . "<td><a href='obradaKorisnika.php?id=" . $row["ID_korisnik"] . "&akcija=1'>OTKLJUČAJ</a></td>" . "<td><a href='obradaKorisnika.php?id=" . $row["ID_korisnik"] . "&akcija=0'>ZAKLJUČAJ</a></td>";
    }
    if ($row["status"] == '1') {
        $table = $table . "<td>" . $row["ime"] . " " . $row["prezime"] . "</td>" . "<td>" . $row["email"] . "</td>" . "<td>" . $row["korisnicko_ime"] . "</td>" . "<td>Otključan</td>" . "<td><a href='obradaKorisnika.php?id=" . $row["ID_korisnik"] . "&akcija=1'>OTKLJUČAJ</a></td>" . "<td><a href='obradaKorisnika.php?id=" . $row["ID_korisnik"] . "&akcija=0'>ZAKLJUČAJ</a></td>";
    }
    $table = $table . "</tr>";
}
